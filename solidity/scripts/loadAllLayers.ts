...
// const contract = new ethers.Contract(addr, abiNouns, wallet);
// const read = async () => {
//   console.log("calling");
...
//     const img = fs.readFileSync("layers/nouns/" + currFile, {
//       encoding: "base64",
//     });
...
//     collectionTraits.rootHash = aPath
//       .substring(aPath.indexOf("/Qm"))
//       .split("/")[1];
  ...
  Object.entries(r.data.traits).forEach(([traitType, traits]) => {
    traits.forEach((trait, i) => {
      ...
      const get = async () => {
        ...
        await sharp(buffer)
          .toFormat("png")
          .toFile(__dirname + "/../localArt/" + trait.url);
      };
      promises.push(get());
    });
  });
